...
This simulation will capture this process by tracking infection dynamics and antibody profiles to multiple strains for 100 individuals across a 45 year period, demonstrating how complex antibody profiles, or antibody landscapes, build over the life course [@Fonville2014, @Lessler2012]. A key concept for this simulation is antigenic space -- a framework for quantifying the antigenic relatedness of strains from the perspective of the immune response [@Smith2004].
...
# 1.1 A/H3N2 influenza epidemiology and simulation settings
...
# 1.2 Exposure to biomarker mapping
...
successful infection depends on the biomarker quantity, 
...
Here we placed no limit on the maximum number of infections; however our selected immunity model will take into 
account an individual's current biomarker level when determining the probability 
of a successful infection. 

Again, the following biomarker-mediated protection parameters 
...
## The immunity model we selected will take into account an individual's current biomarker quantity when determining the probability of successful infection. 
```
...
plot_antibody_model(antibody_model_monophasic_cross_reactivity, N=25, model_pars=model_pars,draw_parameters_fn = draw_parameters_random_fx, biomarker_map=antigenic_map) + theme(strip.text=element_text(size=7),axis.text=element_text(size=6))
```
...
```{r, fig.dim = c(8, 6)} 
## Plot influenza strain A specific biomarker kinetics and exposure histories for 10 individuals 
plot_subset_individuals_history(res$biomarker_states, res$immune_histories_long, subset=10, demography, removal=TRUE, heatmap=TRUE)
```
