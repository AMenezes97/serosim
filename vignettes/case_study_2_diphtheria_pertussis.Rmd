---
title: "Case study 2: Multi-pathogen system with multivalent vaccinaton (Diphtheria and Pertussis)"
...
specific biomarkers for all individuals. We will set up each of the required 
...
pertussis IgG titer.In this example, biomarker quantity, titer level and antibody level all mean the same thing. 
...
Note: The _runserosim_ function, called later on, only requires a demography 
...
_runserosim_ requires that exposure_id and biomarker_id are numeric so we will use 
...
two groups, there are 2 rows in the first foe_pars dimension to specify the group
specific force of exposure.
...
t while the probability of successful infection is dependent on the biomarker quantity, in this case antibody titer, at the 
...
of reinfections by adjusting the biomarker-mediated protection (also known as titer-mediated protection) parameters or by 
...
Now we specify how observed biomarker quantities, in this case antibody titers, are generated as a probabilistic function of the true, latent biomarker quantities, and when to observe these quantities. In this step, users are specifying the sampling design and assay choice for 
...
```{r, fig.dim = c(6, 4)} 
## Plot diphtheria biomarker kinetics and immune histories for 10 individuals 
plot_subset_individuals_history(res$biomarker_states %>% filter(b==1), res$immune_histories_long %>% filter(x==1 | x==3), subset=10, demography, removal=TRUE)
```

```{r, fig.dim = c(6, 4)} 
## Plot pertussis biomarker kinetics and immune histories for 10 individuals 
plot_subset_individuals_history(res$biomarker_states %>% filter(b==2), res$immune_histories_long %>% filter(x==2 | x==3), subset=10, demography, removal=TRUE)
```
...
plot_exposure_histories(res$immune_histories_long)
```
