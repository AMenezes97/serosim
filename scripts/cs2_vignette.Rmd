...
# 1.3 Biomarker Map 
Now, we have to set up the exposure IDs and biomarker IDs for our desired simulation. 
...
biomarker_map_original <- tibble(exposure_id=c("DP_vacc","DP_vacc","diphtheria_ifxn","pertussis_ifxn"),biomarker_id=c("diphtheria_IgG","pertussis_IgG","diphtheria_IgG","pertussis_IgG"))
...
## Plot titer-mediated protection curve given parameters specified within model_pars for biomarker 1 (diphtheria) which will be loaded in section 1.6
plot_titer_mediated_protection(0:20, titer_prot_midpoint=7, titer_prot_width=.5)

## Plot titer-mediated protection curve given parameters specified within model_pars for biomarker 2 (pertussis) which will be loaded in section 1.6
...
the antibody model, must be specified within model_pars. runserosim requires that exposure_id and biomarker_id are numeric within model_pars so we will use the reformat_model_pars function to create a new version of model_pars similarly to what we did with the biomarker_map argument. 
...
## Plot titer dependent boosting effects given parameters specified within model_pars for biomarker 1 (diphtheria)
...
## Plot titer dependent boosting effects given parameters specified within model_pars for biomarker 1 (pertussis)
...
“obs_sd” parameter for each biomarker.
```{r}
...
## Specify observation_times to observe both biomarkers (aka diphtheria and pertussis antibody titer) across all individuals at the end of the simulation (t=120)
