...
serosurvey.In this simulation individuals can either be vaccinated with a 
vaccine containing both antigens or naturally infected with each antigen, or a 
...
status, location, birth and removal time. We will use the generate_pop_demography 
...
aux <- list("SES"=list("name"="SES","options"=c("low","medium","high"), "distribution"=c(0.3,0.3,0.4)),
...
lambdas <- array(n_distinct(demography$group), dim=c(1,max(times),n_distinct(antigen_map$exposure_id)))
...
## Specify the age at which an individual is eligible for vaccination (2 months old for DP vaccine)
vacc_age<-2
...
also incorporates titer-celing effects where an individual’s realized boost is 
...
the latent antibody titer as the mean and the measurement error as the standard 
deviation. The observation standard deviation and distribution is defined within 
theta as the “obs_sd” parameter.
```{r}
