...
cross-sectional serosurvey. The overall structure and principles discussed within 
this case study are also applicable to other vaccine preventable pathogens delivered in multi-valent vaccines. 
In this simulation individuals can either be vaccinated with a 
...
1. Diphtheria
2. Pertussis
runserosim requires that exposure_id and antigen_id are numeric so we will use the reformat_antigen_map function to create a new version of the antigen map. Users can go directly to numeric antigen_map if they wish.
```{r}
## Create antigen map
antigen_map_original <- tibble(exposure_id=c("DP_vacc","DP_vacc","diphtheria_ifxn","pertussis_ifxn"),antigen_id=c("diphtheria_IgG","pertussis_IgG","diphtheria_IgG","pertussis_IgG"))
...
exposure (1-e-λ) depends on the force of exposure (λ) at the current time t for 
...
## Specify the force of exposure for exposure ID 1 which represents diphtheria and pertussis vaccination
foe_pars[1,,1] <- 0.2 ## Group 1 (aka Location 1)
foe_pars[2,,1] <- 0.3 ## Group 2 (aka Location 2)

## Specify the force of exposure for exposure ID 2 which represents diphtheria natural infection
foe_pars[1,,2] <- 0.4 ## Group 1 (aka Location 1)
foe_pars[2,,2] <- 0.3 ## Group 2 (aka Location 2)

## Specify the force of exposure for exposure ID 2 which represents pertussis natural infection
foe_pars[1,,3] <- 0.2 ## Group 1 (aka Location 1)
foe_pars[2,,3] <- 0.1 ## Group 2 (aka Location 2)
...
within this model are defined within model_pars which will be loaded in section 1.6. 
In this simulation an individual can still be "reinfected" with either diphtheria or pertussis. 
These "reinfection" events are representative of boosting events which can occur if an individual is exposed to either pathogen following a vaccination or previous infection. The user can limit the frequency of multiple infections(aka boosting events)
by adjusting the titer-mediated protection parameters within model_pars.
...
## Specify the time step after birth at which an individual is eligible for vaccination (2 months old for DP vaccine); ; note non vaccine exposures are listed as NAs
vacc_age<-c(2,NA,NA)
...
## Plot titer-mediated protection curve given parameters specified within model_pars for antigen 1 (diphtheria) which will be loaded in section 1.6
plot_titer_mediated_protection(0:20, titer_prot_midpoint=7, titer_prot_width=.5)

## Plot titer-mediated protection curve given parameters specified within model_pars for antigen 2 (pertussis) which will be loaded in section 1.6
...
Users can edit the csv file to adjust any parameters. All parameters needed for 
the antibody model, must be specified within model_pars. runserosim requires that exposure_id and antigen_id are numeric within model_pars so we will use the reformat_model_pars function to create a new version of model_pars similarly to what we did with the antigen_map argument. 
Users can go directly to numeric model_pars if they wish. Lastly, we define
the draw_parameters function which determines how each individual’s antibody kinetics 
parameters are simulated from the antibody kinetics parameters tibble (model_pars). 
We will use a function which draws parameters directly from model_pars for the antibody 
model with random effects. Parameters are drawn randomly from a distribution 
with mean and standard deviation specified within model_pars. This draw_parameters function
...
## Plot titer dependent boosting effects given parameters specified within model_pars for antigen 1 (diphtheria)
...
## Plot titer dependent boosting effects given parameters specified within model_pars for antigen 1 (pertussis)
...
observation standard deviation and distribution is defined within model_pars as the 
“obs_sd” parameter for each antigen.
```{r}
## Specify the limits of detection for each antigen for the continuous assays
bounds<-tibble(antigen_id=c(1,1,2,2),name=rep(c("lower_bound","upper_bound"),2),value=c(2,50,4,100))
