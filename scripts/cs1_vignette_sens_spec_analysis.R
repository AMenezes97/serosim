...
thresholds <- seq(from=100, to=350,by=1)
...
ggplot(sens_spec, aes(x=threshold)) + 
  geom_line(aes(y=sensit, col="Sensitivity"), size=1.5) + 
  geom_line(aes(y=specif, col="Specificity"), size=1.5) + 
  labs(title="Sensitivity and specificity for varying thresholds of seropositivity", 
       subtitle="Case study 1: Measles", x="Titer Threshold of Seropositivity (mIU/mL)", y="Percentage") +  # title and caption
  ...
  theme(legend.position = c(0.865, 0.57)) +
