...
individuals can either be vaccinated against the pathogen or naturally infected 
with the pathogen, or a combination of both. We will set up each of the required 
...
will use an ELISA kit to measure an individual's measles IgG titer.
...
example of how users can use the literature to inform required serosim inputs. 
...
measles vaccination (exposure_ID=measles_vacc) both of which will boost titers 
to the same biomarker(biomarker_ID=measles_IgG). Since we are only interested 
...
is dependent on the titer at the time of exposure and an individual's total 
...
prior vaccination if their current titer is below the threshold of protection as 
a result of waning or incomplete  immunity following vaccination. The 
titer-mediated protection parameters used within this immunity model are defined 
within model_pars which will be loaded in section 1.6. 

The Enzygnost ELISA kit reports that the threshold of seropositivity is 200mIU/ml 
and we have used this titer value to inform our titer-mediated protection curve. 
In our simulation, individuals with titers below 200mIU/ml will become infected 
when exposed to measles while all individuals with titers above 200mIU/ml are 
protected from infection. It is important to note that, seropositivty and titer 
...
## Specify the maximum number of successful exposure events an individual can receive for each exposure types
## For this example, we are assuming that there is only one dose of the measles vaccine and one possible measles infection
max_events<-c(1,1)

## Plot titer-mediated protection curve given parameters specified within model_pars for biomarker 1 (measles_IgG) which will be loaded in section 1.6
plot_titer_mediated_protection(0:1000, titer_prot_midpoint=200, titer_prot_width=1)
```

# 1.6  Antibody Model, Antibody Kinetics Parameters, and draw_parameters
Now, we specify the antibody model to be used within runserosim which will 
describe the within-host antibody kinetics following measles infection or 
vaccination event. 
...
titer, we picked the conservative range of 2500-4500 mIU/mL for vaccine acquired 
...
[@Berry2017-ci; @Lyamuya1999-nf; @Carryn2019-qv; @Trevisan2021-of].Since we are only simulating a 10 year period, 
assumed that there was minimal waning of the long-term boost for simplicity.
...
IgG test kits which does not distinguish between neutralizing and non 
neutralizing titers.
...
titer-dependent boosting effects to only allow for the increase in titer 
following a secondary successful exposure event to be only 10% of the regular 
boost which they would have received if their titer level was 0.
...
they run the risk of experiencing a natural infection where there titers will 
...
model_pars_path <- system.file("extdata", "model_pars_cs1_measles.csv", package = "serosim")
...
## Plot titer dependent boosting effects given parameters specified within model_pars for biomarker 1 (measles_IgG)
plot_titer_dependent_boosting(start=0, end=1000, by=1, titer_ceiling_threshold=200, titer_ceiling_gradient=.0045)
...
## Plot individual the probability of exposure for all exposure types.
...
## In other words, this is the probability of exposure event being successful and inducing 
