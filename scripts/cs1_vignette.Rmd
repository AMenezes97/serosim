...
Here, we will simulate measles natural infection(exposure_ID=measles_ifxn) and one vaccine (exposure_ID=measles_vacc) both of which will boost titers to the same antigen(antigen_ID=measles_IgG). 
Since we are only interested in tracking measles antibody levels across time, we only need 
one antigen ID. runserosim requires that exposure_id and antigen_id are numeric so we will use the reformat_antigen_map function to create a new version of the antigen map. Users can go directly to numeric antigen_map if they wish.
```{r}
## Create antigen map
antigen_map_original <- tibble(exposure_id=c("measles_ifxn","measles_vacc"),antigen_id=c("measles_IgG","measles_IgG"))
