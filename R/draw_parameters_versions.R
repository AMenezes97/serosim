...
#'
#' @examples
draw_parameters_V1 <- function(i, t, e, demography, theta, antibody_states, ...){
  ...
  all_pars <- tibble(i=rep(i,nrow(theta_tmp)),t=rep(t,nrow(theta_tmp)), e=rep(e,nrow(theta_tmp)), ag=theta_tmp$antigen_id, name=par_names, value=pars) 
  return(all_pars)
}

#' Draw Parameters Version 2 
#'  
...
#'
#' @examples
draw_parameters_V2 <- function(i, t, e, demography, theta, antibody_states, ...){
  ## Filter for only exposure stimulated 
  theta_tmp <- theta %>% filter(exposure_id == e)
  pars <- numeric(nrow(theta_tmp))
  par_names <- character(nrow(theta_tmp))
  ...
  for(par in 1:nrow(theta_tmp)){
    ...
    if(par_names[par] %in% c("boost_short","boost_long")){
      ...
      pars[par] <- pars[par]*(1-theta_tmp[theta_tmp$name=="titer_ceiling_gradient" & theta_tmp$antigen_id==Antigen, "mean"]*titer_threshold)
    }
  }
  # all_pars <- tibble(i=i, t=t, e=e, ag=ag, name=par_names, value=pars) ## This line doesn't work
  all_pars <- tibble(i=rep(i,nrow(theta_tmp)),t=rep(t,nrow(theta_tmp)), e=rep(e,nrow(theta_tmp)), ag=theta_tmp$antigen_id, name=par_names, value=pars) 
...
#'
#' @examples
draw_parameters_V3 <- function(i, t, e, demography, theta, antibody_states, ...){
  ## Filter for only exposure stimulated 
  theta_tmp <- theta %>% filter(exposure_id == e)
  pars <- numeric(nrow(theta_tmp))
  par_names <- character(nrow(theta_tmp))
...
#'
#' @examples
draw_parameters_V4 <- function(i, t, e, demography, theta, antibody_states, ...){
  ## Filter for only exposure stimulated 
  theta_tmp <- theta %>% filter(exposure_id == e)
  pars <- numeric(nrow(theta_tmp))
  par_names <- character(nrow(theta_tmp))
