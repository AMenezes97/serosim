...
#'
#' @examples
draw_parameters_fixed_fx <- function(i, t, e, ag, demography, antibody_states, model_pars, ...){
...
#'
#' @examples
draw_parameters_random_fx<- function(i, t, e, ag, demography, antibody_states, model_pars, ...){
  ## Filter for only exposure stimulated 
  model_pars_tmp <- model_pars %>% filter(exposure_id == e)
  pars <- numeric(nrow(model_pars_tmp))
  par_names <- character(nrow(model_pars_tmp))
...
#'
#' @examples
draw_parameters_fixed_fx_titer_dep <- function(i, t, e, ag, demography, antibody_states, model_pars, ...){
  ## Filter for only exposure stimulated 
  model_pars_tmp <- model_pars %>% filter(exposure_id == e)
  pars <- numeric(nrow(model_pars_tmp))
...
#'
#' @examples
draw_parameters_random_fx_titer_dep <- function(i, t, e, ag, demography, antibody_states, model_pars, ...){
  ## Filter for only exposure stimulated 
  model_pars_tmp <- model_pars %>% filter(exposure_id == e)
  pars <- numeric(nrow(model_pars_tmp))
  realized <- numeric(nrow(model_pars_tmp))
