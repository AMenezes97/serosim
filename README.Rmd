...
Now, we specify the lambdas argument which contains the force of exposure
...
1 in lambdas. Groups can be used as an indicator of location if the user wishes to 
specify a location specific force of exposure. We specified the same value for all time steps within lambdas for simplicity but users will likely have varying numbers to match real world settings. 
```{r}
...
the user specified antibody model, must be specified within the antibody kinetics parameters tibble (theta). 
...
with mean and standard deviation specified within theta.
```{r}
...
observation standard deviation and distribution are defined within theta as the “obs_sd” parameter.
```{r}
